...
class _HavenCentralScreenState extends State<HavenCentralScreen> {
    ...
    return Container(
        ...
        child: ListView(padding: const EdgeInsets.all(16), children: [
                    ...
                    IconButton(
                        ...
                        onPressed: () {
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                              content: Text(
                                  'Edit Widgets - tap to hide/show coming next update'),
                              backgroundColor: Colors.grey));
                        }),
                  ])),
          const SizedBox(height: 16),
          Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
            _buildToggleTab('🌱 Breeding', _currentIndex == 1, () {
              setState(() => _currentIndex = 1);
            }),
            _buildToggleTab('💰 Finances', _currentIndex == 2, () {
              setState(() => _currentIndex = 2);
            }),
            _buildToggleTab('📋 Records', _currentIndex == 3, () {
          ...
          Row(children: [
            ...
            const SizedBox(width: 12),
            Expanded(
                child: _buildActionCard('👁', 'View All', () {
              setState(() => _currentIndex = 2);
            })),
          ]),
          const SizedBox(height: 16),
  ...
  List<String> _generateAIInsights(
    ...
    if (net > 0) {
      ...
      insights.add(
          '⚠ You\'re spending more than you earn. Consider cutting back on "${catExp.keys.first}" if possible.');
    } else {
          ...
          Row(children: [
            ...
            Expanded(
                child: TextField(
                    ...
                    decoration: InputDecoration(
                        labelText: 'Weight ($unit) optional',
            ...
            ElevatedButton(
                onPressed: () async {
                  ...
                  final weight =
                      double.tryParse(_farmWeightController.text.trim());
  ...
  Widget _buildBudgetTab(List<Budget> budgets, List<Transaction> transactions) {
    ...
    return Container(
        ...
        child: ListView(padding: const EdgeInsets.all(16), children: [
          ...
          const SizedBox(height: 4),
          Text(
              'Month: ${DateFormat('MMMM yyyy').format(DateTime.now())} • Income this month: \$${incomes.toStringAsFixed(2)}',
              style: const TextStyle(color: Colors.grey)),
          const SizedBox(height: 8),
          Text(
              'Suggested budget: Spend < 80% of income = \$${(incomes * 0.8).toStringAsFixed(2)}',
        ...
        child: Row(children: [
          ...
          Expanded(
              child: Column(
                ...
                Text('${tx.place} • ${tx.formattedDate}',
                    style: const TextStyle(color: Colors.grey, fontSize: 12))
              ])),
          ...
          IconButton(
              ...
              onPressed: () =>
                  _showAddTransactionSheet(existingTransaction: tx),
              tooltip: 'Edit'),
          IconButton(
              icon: const Icon(Icons.delete, color: Colors.red, size: 18),
                              ...
                              child: ElevatedButton(
                                  onPressed: () async {
                                    ...
                                    final principal = double.tryParse(
                                        _loanPrincipalController.text.trim());
